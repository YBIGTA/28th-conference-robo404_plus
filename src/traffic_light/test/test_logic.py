...
def test_color_outside_bbox_is_ignored():
    image = make_image((0, 0, 255))
    image[40:60, 40:60] = (0, 0, 0)
    detections = [make_detection()]
...
def test_red_class_name_without_traffic_light_bbox_returns_unknown():
    detections = [make_detection(class_name="red", score=0.9)]
    assert analyze_traffic_light(detections, make_image((0, 0, 255))) == STATE_UNKNOWN


def test_green_class_name_without_traffic_light_bbox_returns_unknown():
    detections = [make_detection(class_name="green", score=0.9)]
    assert analyze_traffic_light(detections, make_image((0, 255, 0))) == STATE_UNKNOWN
