...
MIN_AREA_TRACK = 50 # 5000 original

# Robot's speed when following the line
LINEAR_SPEED = 0.01

# Proportional constant applied to pixel error for turning.
# Keep this small because angular.z is combined with forward speed by the motor controller.
KP = 0.0012

# Derivative constant applied to error rate. The angular output is capped below,
# so transient derivative spikes cannot overpower the drivetrain.
KD = 0.8 / 100

# Minimum speed ratio used by dynamic speed scaling in sharp turns.
MIN_SPEED_SCALE = 0.2

# Absolute angular.z limit. This prevents one wheel/side from overpowering
# the small forward speed and making the car jerk or spin.
MAX_ANGULAR_Z = 0.08
...
upper_bgr_values = np.array([60, 60, 60])
...
def timer_callback():
    ...
    # Smooth the cropped image first to reduce pixel noise and shadow flicker.
    blurred_crop = cv2.GaussianBlur(crop, (5, 5), 0)
    # (filter the color values so only the contour is seen)
    mask = cv2.inRange(blurred_crop, lower_bgr_values, upper_bgr_values)
    ...
    # (as the camera could not be reading any lines)      
        ...
        error = x - width//2
        error_derivative = (float(error) - float(last_error)) / TIMER_PERIOD

        normalized_error = min(abs(float(error)) / max(float(width) / 2.0, 1.0), 1.0)
        speed_scale = max(MIN_SPEED_SCALE, 1.0 - normalized_error)
        message.linear.x = float(LINEAR_SPEED * speed_scale)

        raw_angular_z = (float(error) * -KP) + (error_derivative * -KD)
        message.angular.z = float(clamp(raw_angular_z, -MAX_ANGULAR_Z, MAX_ANGULAR_Z))
    ...
    if not line:
        raw_angular_z = float(error) * -KP
        message.angular.z = float(clamp(raw_angular_z, -MAX_ANGULAR_Z, MAX_ANGULAR_Z))

    print(
        "Error: {} | Raw Angular Z: {} | Clamped Angular Z: {}, ".format(
            error, raw_angular_z, message.angular.z
        )
    )
