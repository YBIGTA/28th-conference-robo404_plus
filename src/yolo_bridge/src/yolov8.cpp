...
#ifndef USE_CUDA
// ==============================================================================
// OpenCV DNN fallback implementations for CPU inference
// ==============================================================================
YOLOv8::YOLOv8(const std::string& engine_file_path) {
    std::cout << "Loading ONNX model for CPU inference: " << engine_file_path << std::endl;
    try {
        this->net = cv::dnn::readNetFromONNX(engine_file_path);
        this->net.setPreferableBackend(cv::dnn::DNN_BACKEND_OPENCV);
        this->net.setPreferableTarget(cv::dnn::DNN_TARGET_CPU);
    } catch (const cv::Exception& e) {
        std::cerr << "\n[ERROR] OpenCV failed to load the ONNX model!\n"
                  << "This usually happens because OpenCV 4.2.0 (default in Ubuntu 20.04/ROS 2 Foxy)\n"
                  << "is too old to parse modern YOLO ONNX models.\n"
                  << "Error details: " << e.what() << "\n"
                  << "-> To fix this in a VM, you need to upgrade OpenCV to >= 4.8.0 or use a simplified ONNX model.\n"
                  << "-> On your real Jetson Nano, the TensorRT (.engine) will work fine.\n" << std::endl;
    }
}
...
void YOLOv8::Infer() {
    if (this->net.empty()) return;
    this->net.forward(dnn_outputs, this->net.getUnconnectedOutLayersNames());
}

void YOLOv8::PostProcess(std::vector<Object>& objs, float score_thres, float iou_thres, int topk, int num_labels) {
    objs.clear();
    if (dnn_outputs.empty()) return;
    
    cv::Mat output = dnn_outputs[0]; 
    if (output.dims != 3) return;

    cv::Mat out_2d(output.size[1], output.size[2], CV_32F, output.ptr<float>());
    out_2d = out_2d.t();

    auto num_anchors = out_2d.rows;
    ...
    for (int i = 0; i < num_anchors; i++) {
        auto row_ptr = out_2d.row(i).ptr<float>();
        auto bboxes_ptr = row_ptr;
        auto scores_ptr = row_ptr + 4;
        auto max_s_ptr  = std::max_element(scores_ptr, scores_ptr + num_labels);
        float score      = *max_s_ptr;
